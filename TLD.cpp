...
void TLD::generateNegativeData(const Mat& frame) {
    ...
    std::shuffle(std::begin(bad_boxes), std::end(bad_boxes), generator);
...
void TLD::detect(const cv::Mat& frame) {
    ...
    cvtColor(img,img,COLOR_GRAY2BGR);
      for (int i=0;i<detections;i++)
      {
          ...
          rt.x = grid[dt.bb[i]].x;
          rt.y = grid[dt.bb[i]].y;
          rt.width = grid[dt.bb[i]].width;
          rt.height = grid[dt.bb[i]].height;
          drawBox(img,rt,cv::Scalar(255,255,0),1);
      }
    ...
    for (int i=0;i<dbb.size();i++)
      {
          ...
          rt.x = dbb[i].x;
          rt.y = dbb[i].y;
          rt.width = dbb[i].width;
          rt.height = dbb[i].height;
          drawBox(img,rt,cv::Scalar(0,0,255),2);
      }
    imshow("detections",img);
      waitKey(1);
}
